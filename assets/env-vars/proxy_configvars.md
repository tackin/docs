...
|`PROXY_TRANSPORT_TLS_CERT`| 1.0.0 |string|Path/File name of the TLS server certificate (in PEM format) for the external http services. If not defined, the root directory derives from $OC_BASE_DATA_PATH/proxy.|/home/opencloud/.opencloud/proxy/server.crt|
|`PROXY_TRANSPORT_TLS_KEY`| 1.0.0 |string|Path/File name for the TLS certificate key (in PEM format) for the server certificate to use for the external http services. If not defined, the root directory derives from $OC_BASE_DATA_PATH/proxy.|/home/opencloud/.opencloud/proxy/server.key|
